...
class PartyController extends Controller
{
    ...
    {
        $metatitle = "Category";
        // $Partys = Party::with('category')->where('status', '=', 1)->get();



        return view('Admin.Party.index', compact('metatitle'));
    }
    ...
    //         $records = Party::with('category')->whereIn('status', [0, 1]);
    ...
    //             $url = '';
    //             if (!empty($data->image) && File::exists(public_path('uploads/Partys/' . $data->image))) {
    //                 $url = url('public/uploads/Partys/thumbnails/' . $data->image);
    //             }
    ...
    //                         <a href="' . route('Party.edit', $data->id) . '" class="me-2 p-2 text-success">
    ...
    public function create()
    {
        ...
        return view('Admin.Party.create', compact('categorys'));
    }
    ...
    //         $uploadPath = public_path('uploads/Partys');
    //         $thumbPath = public_path('uploads/Partys/thumbnails');
    ...
    //     return redirect()->route('Party')->with('success', 'Site settings saved successfully!');
    // }
    ...
    //     return view('Admin.Party.edit', compact('category', 'Party'));
    // }
    ...
    //     if ($request->hasFile('image')) {
    //         $oldImagePath = public_path('uploads/Partys/' . $Party->image);
    //         $oldThumbPath = public_path('uploads/Partys/thumbnails/' . $Party->image);
    //         if (!empty($Party->image)) {
    ...
    //         $basePath = 'uploads/Partys/';
    //         $thumbPath = 'uploads/Partys/thumbnails/';
    ...
    //         ]);
    //     } catch (\Exception $e) {
    //         \Log::error('Status update failed for sub$Party ID ' . $id . ': ' . $e->getMessage());

    //         return response()->json([
    //             'success' => false,
    //             'message' => 'Failed to update sub$Party status.'
    //         ], 500);
    //     }
    // }
}
